...
notify_operator(){
  ...
  local detail="$2"
  local msg
  msg="[oscar-hermes-weekly-maintenance] ${level} | mode=${MODE} | reason=${defer_reason:-none} | detail=${detail} | log=${LOG_PATH}"
...
check_preflight(){
  ...
  # Do not hard-require private-control-repo fork/upstream reachability unless a separate
  ...
  if git -C "$REPO_DIR" ls-remote --heads origin main >/dev/null 2>&1; then
    say "OK: private-control-repo fork reachability"
  else
    say "WARN: private-control-repo fork reachability check failed (non-blocking for Hermes auto-update)"
  fi

  if git -C "$REPO_DIR" ls-remote --heads upstream main >/dev/null 2>&1; then
    say "OK: private-control-repo upstream reachability"
  else
    say "WARN: private-control-repo upstream reachability check failed (non-blocking for Hermes auto-update)"
  fi
...
# Project defaults from register (private-control-repo)
...
OSCAR_MAINT_ALLOWLIST_REGEX="${OSCAR_MAINT_ALLOWLIST_REGEX:-^(docs/hermes-update-policy\.md|scripts/hermes-weekly-update-check\.sh|scripts/oscar-hermes-weekly-maintenance\.sh|docs/operating-model\.md|policy/github-safety\.md|scripts/templates/oscar-hermes-weekly-maintenance\.service|scripts/templates/oscar-hermes-weekly-maintenance\.timer)$}"
...
run_guarded_git_maintenance() {
  ...
  if [[ "$dry_run_mode" == "1" ]]; then
    ...
    echo "DRY RUN PR target: Eurobotics-Association/private-control-repo:${PR_BASE_BRANCH} <- runtime-agentaiagent-creator:${branch}"
  ...
  if gh auth status >/dev/null 2>&1; then
    if gh pr view --repo Eurobotics-Association/private-control-repo "$branch" >/dev/null 2>&1; then
      gh pr edit --repo Eurobotics-Association/private-control-repo "$branch" \
      ...
      gh pr create --repo Eurobotics-Association/private-control-repo \
        --base "$PR_BASE_BRANCH" \
        --head "runtime-agentaiagent-creator:$branch" \
        --title "chore: weekly Hermes maintenance sync (${TS_UTC})" \
        --body "Automated weekly maintenance sync from Oscar fork branch ${branch}.\n\nPolicy guards enforced:\n- allowlist-only file commit\n- no secret-like staged content\n- no merge action\n\nPlease review before merge."
...
echo "Scheduled guarded auto-update is handled by /usr/local/bin/oscar-hermes-weekly-maintenance --auto-update"
echo "Log path: $(redact_path "$LOG_PATH")"
...
Oscar/Hermes should be checked weekly for updates.
...
- `scripts/oscar-hermes-weekly-maintenance.sh`
...
- private-control-repo fork/upstream reachability checks are non-blocking for Hermes auto-update and only blocking inside explicit git-maintenance/PR workflows.
...
- service: `~/.config/systemd/user/oscar-hermes-weekly-maintenance.service`
- timer: `~/.config/systemd/user/oscar-hermes-weekly-maintenance.timer`
- schedule: `OnCalendar=Sat *-*-* 01:00:00 Europe/Paris`
- mode: guarded auto-update (`ExecStart=/usr/local/bin/oscar-hermes-weekly-maintenance --auto-update`)
...
- `/usr/local/bin/oscar-hermes-weekly-maintenance --check-only`
- `/usr/local/bin/oscar-hermes-weekly-maintenance --dry-run --auto-update`
- `/usr/local/bin/oscar-hermes-weekly-maintenance --auto-update`

Template sources are stored in repo:

- `scripts/templates/oscar-hermes-weekly-maintenance.service`
- `scripts/templates/oscar-hermes-weekly-maintenance.timer`
- `scripts/oscar-hermes-weekly-maintenance.sh`
...
When maintaining `/usr/local/bin/oscar-hermes-weekly-maintenance`, keep the existing safety model but prevent false defers and silent outcomes.
...
1. Confirm `oscar-backup` repo presence and cleanliness under `<private-workspace-path>`.
...
- Syntax: `bash -n /usr/local/bin/oscar-hermes-weekly-maintenance`
- Dry-run: `/usr/local/bin/oscar-hermes-weekly-maintenance --dry-run --auto-update`
